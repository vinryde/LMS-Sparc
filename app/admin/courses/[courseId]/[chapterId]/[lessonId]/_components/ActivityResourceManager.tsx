...
import {
  ...
  getActivityResourcesData, // added
} from "../actions";
...
interface ActivityResourceManagerProps {
  activityId: string;
  initialResources: ResourceData[];
  onResourceUpdate?: (resources: ResourceData[]) => void;
}

export function ActivityResourceManager({ 
  activityId, 
  initialResources, 
  onResourceUpdate 
}: ActivityResourceManagerProps) {
  ...
  async function handleDeleteResource(resourceId: string) {
    const { data: result, error } = await tryCatch(
      deleteActivityResource({ resourceId, activityId })
    );
  ...
  async function handleResourceSaved() {
    const { data: resourcesData } = await tryCatch(getActivityResourcesData(activityId));
    if (resourcesData) {
      setResources(resourcesData || []);
      if (onResourceUpdate) {
        onResourceUpdate(resourcesData);
      }
    }
    ...
    <div className="space-y-6">
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h4 className="text-sm font-semibold">Activity Resources</h4>
...
function SortableActivityResource({
  resource,
  onEdit,
  onDelete,
  isEditing,
...
function ActivityResourceEditor({
  activityId,
  resource,
  onSave,
  onCancel,
  ...
  async function handleSave() {
    ...
    const { data: result, error } = resource
      ? await tryCatch(updateActivityResource(payload, resource.id))
      : await tryCatch(createActivityResource(payload));
